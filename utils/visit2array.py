...
def visit2array_train(path):
    ...
    length = len(filenames)
    start_time = time.time()
    create_dir(r'./cache/visit_npy')
    for index, filename in enumerate(filenames):
        table = pd.read_table(path + '/'+filename, header=None)
        array = visit2array(table)
        np.save("./cache/visit_npy/"+filename.strip('.txt')+".npy", array)
        sys.stdout.write('\r>> Processing visit data %d/%d'%(index+1, length))
        sys.stdout.flush()
    sys.stdout.write('\n')
    print("using time:%.2fs"%(time.time()-start_time))
...
if __name__ == '__main__':
    visit2array_train(r'F:\Chrome-Download\2019bigdata\2019bigdata\train_visit\train')
